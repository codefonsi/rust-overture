use overture_core::concat::{
    concat, concat_mut, concat_ref_mut, concat_throwing, concat_with, concat_with_mut,
    concat_with_ref_mut, concat_with_throwing,
};
...
fn main() {
    ...
    let composed_with = concat_with(vec![|x: i32| x + 1, |x: i32| x * 2, |x: i32| x - 3]);
    println!(
        "concat_with([|x| x+1, |x| x*2, |x| x-3])(2) = {}",
        composed_with(2)
    );
    println!();
    ...
    println!(
        "string_composed(\"world\") = {}",
        string_composed("world".to_string())
    );
    ...
    println!(
        "string_composed_with(\"rust\") = {}",
        string_composed_with("rust".to_string())
    );
    println!();
    ...
    let throwing_functions = vec![
        ...
        Box::new(|x: i32| if x < 100 { Ok(x * 2) } else { Err("too large") })
            as Box<dyn Fn(i32) -> Result<i32, &'static str>>,
    ...
    println!(
        "concat_with_mut([|x| *x+=2, |x| *x*=3, |x| *x-=1])(1) = {}",
        val2
    );
    println!();
    ...
    println!(
        "concat_ref_mut([|x| *x+=2, |x| *x*=3, |x| *x-=1])(1) = {}",
        *val.borrow()
    ); // ((1+2) * 3) - 1 = 8
    ...
    println!(
        "concat_with_ref_mut([|x| *x+=2, |x| *x*=3, |x| *x-=1])(1) = {}",
        *val2.borrow()
    );
    println!();
    ...
    let data_functions = vec![
        Box::new(|mut data: Data| {
            data.value += 10;
            data
        }) as Box<dyn Fn(Data) -> Data>,
        Box::new(|mut data: Data| {
            data.value *= 2;
            data
        }) as Box<dyn Fn(Data) -> Data>,
    ...
    let single_functions = vec![Box::new(|x: i32| x * 2) as Box<dyn Fn(i32) -> i32>];
    let single_composed = concat(single_functions);
    println!("single_composed(5) = {}", single_composed(5)); // Should return 10

    let single_composed_with = concat_with(vec![|x: i32| x * 2]);
    ...
    let concat_result = concat_with(vec![|x: i32| x + 1, |x: i32| x * 2, |x: i32| x - 3]);
