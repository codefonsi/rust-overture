use overture_core::combinig::{
    combining, combining_mut, their, their_add, their_cmp, their_div, their_eq, their_max,
    their_min, their_mul, their_sub,
};
...
fn main() {
    ...
    println!(
        "combining(|u| u.age, |a, b| a + b)(10, &user) = {}",
        f(10, &user)
    );

    let g = combining(|u: &User| u.score, |a, b| a + b);
    println!(
        "combining(|u| u.score, |a, b| a + b)(10.5, &user) = {}",
        g(10.5, &user)
    );
    println!();
    ...
    f(&mut value, &user);
    println!(
        "After combining_mut(|u| u.age, |a, b| *a += b): value = {}",
        value
    );
    ...
    g(&mut score, &user);
    println!(
        "After combining_mut(|u| u.score, |a, b| *a *= b): score = {}",
        score
    );
    println!();
    ...
    println!(
        "their_cmp(|u| u.age)(&alice, &bob) = {}",
        age_lt(&alice, &bob)
    ); // 20 < 25
    println!(
        "their_cmp(|u| u.age)(&bob, &alice) = {}",
        age_lt(&bob, &alice)
    ); // 25 < 20

    let age_eq = their_eq(|u: &User| u.age);
    println!(
        "their_eq(|u| u.age)(&alice, &charlie) = {}",
        age_eq(&alice, &charlie)
    ); // 20 == 20
    println!(
        "their_eq(|u| u.age)(&alice, &bob) = {}",
        age_eq(&alice, &bob)
    ); // 20 == 25
    ...
    println!(
        "their_max(|u| u.age)(&alice, &bob) = {}",
        age_max(&alice, &bob)
    );
    println!(
        "their_min(|u| u.age)(&alice, &bob) = {}",
        age_min(&alice, &bob)
    );
    ...
    println!(
        "their_add(|u| u.age)(&alice, &bob) = {}",
        age_add(&alice, &bob)
    ); // 20 + 25
    println!(
        "their_sub(|u| u.age)(&bob, &alice) = {}",
        age_sub(&bob, &alice)
    ); // 25 - 20
    println!(
        "their_mul(|u| u.age)(&alice, &bob) = {}",
        age_mul(&alice, &bob)
    ); // 20 * 25
    println!(
        "their_div(|u| u.score)(&bob, &alice) = {}",
        score_div(&bob, &alice)
    ); // 90.0 / 85.0
    println!();
    ...
    println!(
        "max rating (phone, tablet): {}",
        rating_max(&phone, &tablet)
    );
    ...
    println!(
        "stock value (laptop, phone): ${}",
        stock_value(&laptop, &phone)
    );
    println!(
        "total stock (laptop, phone): {}",
        total_stock(&laptop, &phone)
    );
    println!();
    ...
    let name_combine = combining(
        |u: &User| u.name.clone(),
        |a: String, b: String| format!("{} + {}", a, b),
    );
    println!(
        "name_combine(\"Hello\", &user) = {}",
        name_combine("Hello".to_string(), &user)
    );

    // Custom score calculation
    let score_combine = combining(|u: &User| u.score, |a: f64, b: f64| (a + b) / 2.0);
    println!(
        "score_combine(90.0, &user) = {}",
        score_combine(90.0, &user)
    );
    ...
    println!(
        "name_combine(Some(\"Hello\".to_string()), &user) = {:?}",
        name_combine(Some("Hello".to_string()), &user)
    );

    let age_combine = combining(get_age, |a: Option<u32>, b: Option<u32>| match (a, b) {
        (Some(a), Some(b)) => Some(a + b),
        _ => None,
    });
    println!(
        "age_combine(Some(10), &user) = {:?}",
        age_combine(Some(10), &user)
    );
    println!();
    ...
    let multi_combine = combining(
        |u: &User| (u.age, u.score),
        |a: (u32, f64), b: (u32, f64)| (a.0 + b.0, a.1 + b.1),
    );
    ...
    let validate_combine = combining(
        |u: &User| u.age,
        |a: u32, b: u32| {
            if a >= 18 && b >= 18 {
                Some(a + b)
    ...
    let transform_combine = combining(
        |u: &User| u.age,
        |a: u32, b: u32| format!("Age {} + {} = {}", a, b, a + b),
    );

    let result = transform_combine(10, &user);
    println!("transform_combine(10, &user) = {}", result);

    let score_transform = combining(
        |u: &User| u.score,
        |a: f64, b: f64| format!("Score: {:.1} + {:.1} = {:.1}", a, b, a + b),
    );
