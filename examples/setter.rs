use overture_core::setter::{
    mset, mset_ref, mut_set, mut_set_ref, mver, mver_ref, mver_ref_mut, over, set,
};
...
fn main() {
    ...
    let setter =
        |f: Box<dyn Fn(i32) -> i32>| Box::new(move |x: i32| f(x)) as Box<dyn Fn(i32) -> i32>;
    ...
    let setter = |f: Box<dyn Fn(String) -> String>| {
        Box::new(move |s: String| f(s)) as Box<dyn Fn(String) -> String>
    };
    let transform = over(setter, |s| s.to_uppercase());
    let result = transform("hello world".to_string());
    println!(
        "over(setter, |s| s.to_uppercase())(\"hello world\") = {}",
        result
    );
    println!();
    ...
    let setter = |mut f: Box<dyn FnMut(&mut i32)>| {
        Box::new(move |x: &mut i32| f(x)) as Box<dyn FnMut(&mut i32)>
    };
    ...
    let setter = |f: Box<dyn Fn(Rc<RefCell<i32>>)>| {
        Box::new(move |x: Rc<RefCell<i32>>| f(x)) as Box<dyn Fn(Rc<RefCell<i32>>)>
    };
    let mut_transform = mver_ref_mut(setter, |x| *x.borrow_mut() *= 2);
    let value = Rc::new(RefCell::new(5));
    mut_transform(value.clone());
    println!(
        "mver_ref_mut(setter, |x| *x.borrow_mut() *= 2) on value 5 = {}",
        *value.borrow()
    );
    println!();
    ...
    let age_setter = |f: Box<dyn Fn(u32) -> u32>| {
        Box::new(move |mut user: User| {
            user.age = f(user.age);
            user
        }) as Box<dyn Fn(User) -> User>
    ...
    let email_setter = |f: Box<dyn Fn(String) -> String>| {
        Box::new(move |mut user: User| {
    ...
    let email_transform = over(email_setter, |email| {
        email.replace("@example.com", "@company.com")
    });
    ...
    let age_mut_setter = |mut f: Box<dyn FnMut(&mut u32)>| {
        Box::new(move |user: &mut User| f(&mut user.age)) as Box<dyn FnMut(&mut User)>
    };
    ...
    let age_ref_setter = |mut f: Box<dyn FnMut(&mut u32)>| {
        Box::new(move |user: Rc<RefCell<User>>| f(&mut user.borrow_mut().age))
            as Box<dyn FnMut(Rc<RefCell<User>>)>
    };
    ...
    let setter = |f: Box<dyn Fn(Result<i32, String>) -> Result<i32, String>>| {
        Box::new(move |x: Result<i32, String>| f(x))
            as Box<dyn Fn(Result<i32, String>) -> Result<i32, String>>
    };
