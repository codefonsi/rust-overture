...
fn main() {
    ...
    println!("\"5\" -> parse_int -> add_ten -> square -> format_result = {}", result);
    println!();
    ...
    let process_text = pipe5(get_length, multiply_by_three, add_five, to_string, add_suffix);
    let result = process_text("Hello");
    println!("\"Hello\" -> get_length -> multiply_by_three -> add_five -> to_string -> add_suffix = {}", result);
    println!();
    ...
    let process_string = pipe6(to_lowercase, add_underscore, add_prefix, get_length, multiply_by_two, format_final);
    let result = process_string("HELLO".to_string());
    println!("\"HELLO\" -> to_lowercase -> add_underscore -> add_prefix -> get_length -> multiply_by_two -> format_final = {}", result);
    println!();
    ...
    let divide_by_two = |x: i32| if x % 2 == 0 { Ok(x / 2) } else { Err("Odd number") };
    ...
    let process_text = pipe5(trim, to_lowercase, replace_commas, add_exclamation, get_length);
