...
fn main() {
    ...
    println!(
        "chain3(parse_int, double, to_string)(\"7\") = {:?}",
        chained3("7")
    );
    println!();
    ...
    let double_throwing = |n: i32| -> Result<Option<i32>, String> { Ok(Some(n * 2)) };

    let throwing_chain = chain3_throwing(
        parse_int_throwing,
    ...
    let square_throwing = |n: i32| -> Result<Vec<i32>, String> { Ok(vec![n * n]) };

    let filter_positive_throwing =
        |n: i32| -> Result<Vec<i32>, String> { if n > 0 { Ok(vec![n]) } else { Ok(vec![]) } };

    let vec_throwing_chain =
        chain3_vec_throwing(expand_throwing, square_throwing, filter_positive_throwing);
    ...
    let user_processing_chain = chain5(
        parse_user_id,
        validate_user_id,
        create_user,
        validate_age,
    ...
    println!(
        "user_processing_chain(\"123\") = {:?}",
        user_processing_chain("123")
    );
    println!(
        "user_processing_chain(\"abc\") = {:?}",
        user_processing_chain("abc")
    );
    println!();
    ...
    let strict_validation_chain = chain5(
        parse_int,
        validate_positive,
        validate_even,
        validate_multiple_of_3,
        format_result,
    );
    println!(
        "strict_validation_chain(\"6\") = {:?}",
        strict_validation_chain("6")
    ); // Passes all
    println!(
        "strict_validation_chain(\"4\") = {:?}",
        strict_validation_chain("4")
    ); // Fails multiple of 3
    println!(
        "strict_validation_chain(\"3\") = {:?}",
        strict_validation_chain("3")
    ); // Fails even
    println!(
        "strict_validation_chain(\"-2\") = {:?}",
        strict_validation_chain("-2")
    ); // Fails positive
    println!();
    ...
    println!(
        "word_chain(\"hello world rust programming\") = {:?}",
        word_chain("hello world rust programming")
    );
    println!();
