use overture_core::compose::{
    compose3, compose3_throwing, compose4, compose4_throwing, compose6
};
...
fn demonstrate_basic_composition() {
    ...
    println!("Trigonometric composition: sin(cos(π + 0)) = {:.6}", trig_result);
}
...
fn demonstrate_user_processing() {
    ...
    let users = vec![
        User { id: 1, name: "Alice Johnson".to_string(), email: "alice@example.com".to_string(), age: 28, is_active: true },
        User { id: 2, name: "Bob Smith".to_string(), email: "bob@example.com".to_string(), age: 35, is_active: false },
        User { id: 3, name: "Charlie Brown".to_string(), email: "charlie@example.com".to_string(), age: 22, is_active: true },
    ];
    ...
    for user in &users {
        ...
        println!("Original: '{}' -> Processed: '{}'", user.name, processed_name);
    }
}
...
fn demonstrate_throwing_composition() {
    ...
    let add_prefix = |category: String| -> Result<String, String> {
        Ok(format!("Age Category: {}", category))
    };
...
fn demonstrate_complex_pipeline() {
    ...
    let raw_users = vec![
        ...
        ("3,Charlie,charlie@example.com,22,true", "music,art,photography"),
    ];
    ...
    let user_processing_pipeline = compose3_throwing(
        finalize_profile,
        calculate_risk,
        parse_user,
    );
...
fn demonstrate_mathematical_composition() {
    ...
    for value in test_values {
        ...
        println!("f({:.1}) = |sin(π * ({:.1} + 1)²)| = {:.6}", value, value, result);
    }
}
...
fn demonstrate_performance_comparison() {
    ...
    println!("Individual function calls: {:?} (result: {:.6})", individual_time, result1);
    println!("Composed function: {:?} (result: {:.6})", composed_time, result2);
    println!("Performance difference: {:.2}%", 
        ((composed_time.as_nanos() as f64 - individual_time.as_nanos() as f64) / individual_time.as_nanos() as f64) * 100.0);
}
