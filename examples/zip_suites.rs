...
use overture_core::suites::{filter, map as seq_map};
...
fn main() {
    ...
    let descriptions = zip3_with(
        |name, age, city| format!("{} (age {}) from {}", name, age, city),
        names,
        ages,
        cities,
    );
    ...
    let employee_profiles = zip7_with(
    ...
    let transaction_details = zip10_with(
    ...
    let processed =
        seq_map(|(n, s, c, sum)| format!("n={}, s={}, c={}, sum={}", n, s, c, sum))(filtered);
    ...
    let validated_users = zip10_with(
        |name, age_str, email, phone, address, city, country, postal, reg_date, last_login| {
    ...
    println!(
        "zip3 with different lengths: {:?}",
        zipped_different_lengths
    );
