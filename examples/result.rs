...
fn main() {
    ...
    let zipped_with = zip_with(
        Ok(5) as Result<i32, &str>,
    ...
    println!(
        "zip3_with(Ok(1), Ok(2), Ok(3), |a, b, c| a * b + c) = {:?}",
        zipped3_with
    );
    println!();
    ...
    println!(
        "zip4_with(Ok(1), Ok(2), Ok(3), Ok(4), |a, b, c, d| a + b + c + d) = {:?}",
        zipped4_with
    );
    println!();
    ...
    let create_user =
        |id: u32, name: String, email: String, age: u32| User::new(id, &name, &email, age);
    ...
    println!(
        "process_user_data(\"123\", \"Alice\", \"alice@example.com\", \"25\") = {:?}",
        user_result
    );

    let error_result = process_user_data("abc", "Bob", "invalid-email", "30");
    println!(
        "process_user_data(\"abc\", \"Bob\", \"invalid-email\", \"30\") = {:?}",
        error_result
    );
    println!();
    ...
    let processed_users: Vec<Result<User, &str>> =
        raw_data.into_iter().map(process_single_user).collect();
    ...
    println!(
        "flat_map(Ok(-5), complex_transform) = {:?}",
        flat_mapped_error
    );
    println!();
    ...
    println!(
        "process_api_response(success_response) = {:?}",
        processed_success
    );
    ...
    let results: Vec<Result<i32, &str>> = numbers.into_iter().map(parse_and_double).collect();
    ...
    let error_messages: Vec<&str> = errors.into_iter().map(|r| r.unwrap_err()).collect();
    ...
    let all_results: Vec<Result<i32, &str>> =
        all_positive.into_iter().map(parse_and_double).collect();
