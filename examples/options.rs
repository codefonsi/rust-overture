...
fn main() {
    ...
    println!(
        "map_throwing(|x| 10/x)(Some(2)) = {:?}",
        safe_divide(Some(2))
    );
    println!(
        "map_throwing(|x| 10/x)(Some(0)) = {:?}",
        safe_divide(Some(0))
    );
    ...
    println!(
        "zip_with(|a, b| a + b, Some(1), Some(2)) = {:?}",
        zip_with(add, Some(1), Some(2))
    );
    println!(
        "zip_with(|a, b| a + b, Some(1), None) = {:?}",
        zip_with(add, Some(1), None::<i32>)
    );
    println!(
        "zip_with(|a, b| a + b, None, Some(2)) = {:?}",
        zip_with(add, None::<i32>, Some(2))
    );
    println!(
        "zip_with(|a, b| a + b, None, None) = {:?}",
        zip_with(add, None::<i32>, None::<i32>)
    );
    println!();
    ...
    let parse_number = map_throwing(|s: String| s.parse::<i32>().map_err(|_| "Invalid number"));
    ...
    println!(
        "parse_number(Some(\"123\")) = {:?}",
        parse_number(Some("123".to_string()))
    );
    println!(
        "parse_number(Some(\"abc\")) = {:?}",
        parse_number(Some("abc".to_string()))
    );
    ...
    println!(
        "zip_with(create_person, Some(\"Alice\"), Some(30)) = {:?}",
        person_info
    );
    ...
    println!(
        "validate_email(Some(\"ALICE@EXAMPLE.COM\")) = {:?}",
        validated_email
    );
