use overture_core::flip::{
    flip, flip_throw, flip_throwing, flip0, flip2, flip2_throwing, flip3, flip3_throwing, flip4,
    flip4_throwing, flip5, flip5_throwing, flip6, flip6_throwing,
};
...
fn main() {
    ...
    let curried =
        |a: i32| Box::new(move |b: i32, c: i32| a + b + c) as Box<dyn Fn(i32, i32) -> i32>;
    ...
    let curried = |a: i32| {
        Box::new(move |b: i32, c: i32, d: i32| a + b + c + d) as Box<dyn Fn(i32, i32, i32) -> i32>
    };
    let flipped = flip4(curried);
    let result = flipped(2, 3, 4)(1);
    println!(
        "flip4(|a| |b, c, d| a + b + c + d)(2, 3, 4)(1) = {}",
        result
    );
    assert_eq!(result, 10);
    println!();
    ...
    let curried = |a: i32| {
        Box::new(move |b: i32, c: i32, d: i32, e: i32, f: i32| a + b + c + d + e + f)
            as Box<dyn Fn(i32, i32, i32, i32, i32) -> i32>
    };
    let flipped = flip6(curried);
    let result = flipped(2, 3, 4, 5, 6)(1);
    println!(
        "flip6(|a| |b, c, d, e, f| a + b + c + d + e + f)(2, 3, 4, 5, 6)(1) = {}",
        result
    );
    assert_eq!(result, 21);
    println!();
    ...
    let result = flipped()(5);
    println!(
        "flip_throwing(|a| || if a == 0 {{ Err(\"Zero not allowed\") }} else {{ Ok(a * 2) }})()(5) = {:?}",
        result
    );
    ...
    println!(
        "flip_throwing(|a| || if a == 0 {{ Err(\"Zero not allowed\") }} else {{ Ok(a * 2) }})()(0) = {:?}",
        error_result
    );
    ...
    println!(
        "flip2_throwing(|a| |b| if b == 0 {{ Err(\"Division by zero\") }} else {{ Ok(a / b) }})(2)(10) = {:?}",
        result
    );
    ...
    println!(
        "flip2_throwing(|a| |b| if b == 0 {{ Err(\"Division by zero\") }} else {{ Ok(a / b) }})(0)(10) = {:?}",
        error_result
    );
    ...
    println!(
        "flip3_throwing(|a| |b, c| if c == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b) / c) }})(2, 3)(10) = {:?}",
        result
    );
    ...
    println!(
        "flip3_throwing(|a| |b, c| if c == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b) / c) }})(2, 0)(10) = {:?}",
        error_result
    );
    assert_eq!(error_result, Err("Division by zero"));
    println!();
    ...
    let curried = |a: i32| {
        Box::new(move |b: i32, c: i32, d: i32| {
            if d == 0 {
                Err("Division by zero")
            } else {
                Ok((a + b + c) / d)
    ...
    println!(
        "flip4_throwing(|a| |b, c, d| if d == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b + c) / d) }})(2, 3, 4)(10) = {:?}",
        result
    );
    ...
    println!(
        "flip4_throwing(|a| |b, c, d| if d == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b + c) / d) }})(2, 3, 0)(10) = {:?}",
        error_result
    );
    assert_eq!(error_result, Err("Division by zero"));
    println!();
    ...
    let curried = |a: i32| {
        Box::new(move |b: i32, c: i32, d: i32, e: i32| {
            if e == 0 {
                Err("Division by zero")
            } else {
                Ok((a + b + c + d) / e)
    ...
    println!(
        "flip5_throwing(|a| |b, c, d, e| if e == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b + c + d) / e) }})(2, 3, 4, 5)(10) = {:?}",
        result
    );
    assert_eq!(result, Ok(3));
    ...
    println!(
        "flip5_throwing(|a| |b, c, d, e| if e == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b + c + d) / e) }})(2, 3, 4, 0)(10) = {:?}",
        error_result
    );
    assert_eq!(error_result, Err("Division by zero"));
    println!();
    ...
    let curried = |a: i32| {
        Box::new(move |b: i32, c: i32, d: i32, e: i32, f: i32| {
            if f == 0 {
                Err("Division by zero")
            } else {
                Ok((a + b + c + d + e) / f)
    ...
    println!(
        "flip6_throwing(|a| |b, c, d, e, f| if f == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b + c + d + e) / f) }})(2, 3, 4, 5, 6)(10) = {:?}",
        result
    );
    assert_eq!(result, Ok(4));
    ...
    println!(
        "flip6_throwing(|a| |b, c, d, e, f| if f == 0 {{ Err(\"Division by zero\") }} else {{ Ok((a + b + c + d + e) / f) }})(2, 3, 4, 5, 0)(10) = {:?}",
        error_result
    );
    assert_eq!(error_result, Err("Division by zero"));
    println!();
    ...
    let result = flipped(2)(10);
    println!(
        "flip_throw(|a| |b| if b == 0 {{ Err(\"Division by zero\") }} else {{ Ok(a / b) }})(2)(10) = {:?}",
        result
    );
    assert_eq!(result, Ok(5));
    println!();
    ...
    let curried = |prefix: String| {
        Box::new(move |suffix: String| format!("{}-{}", prefix, suffix))
            as Box<dyn Fn(String) -> String>
    };
    let flipped = flip2(curried);
    let result = flipped("world".to_string())("hello".to_string());
    println!(
        "flip2(|prefix| |suffix| format!(\"{{}}-{{}}\", prefix, suffix))(\"world\")(\"hello\") = {}",
        result
    );
    ...
    let config_builder = |host: String| {
        Box::new(move |port: u16, timeout: u64| {
