use overture_core::{curry::curry, pipe::pipe3_throwing};
...
fn validate_credit_transfer_transaction(
    ...
    let requested_execution_date = ctti
...
fn create_advanced_validator()
-> impl Fn(&PaymentInitiation) -> ValidationResult<HashMap<String, f64>> {
    move |pi: &PaymentInitiation| {
        // Use functional operations to get only valid transactions
        let valid_transactions = pi
            .payment_information
            .iter()
            .flat_map(|pi| &pi.credit_transfer_transaction_information)
